(ns rapids.partitioner.partition_test
  (:require [clojure.core.match :refer [match]]
            ...
            [rapids.partitioner.methods :refer [partition-map-expr partition-vector-expr]]
...
(deftest ^:unit PartitionMapExpression
  (testing "simple set with scalars"
    (let [[start, pmap, suspend?] (partition-map-expr `#{"str" :key 1} partition-address address [])]
      (is (false? suspend?))
      (is (empty? pmap))
      (is (= start #{"str" :key 1}))))

  (testing "set with function calls"
    (let [[start, pmap, suspend?] (partition-map-expr `#{(a) (b) 3} partition-address address [])]
      (is (= 0 (count pset)))
      (is (false? suspend?))
      (is (= start `#{(a) (b) 3}))))

  (testing "set with suspending expressions"
    (let [[start, pset, suspend?] (partition-map-expr `#{(fl1) (fl2) 3} partition-address address [])]
