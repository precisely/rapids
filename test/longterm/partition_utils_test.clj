(ns longterm.partition-utils-test
  (:require [longterm.partition-utils :refer :all]
...
(deftest ^:unit unqualified-symbols-in-test
  (testing "it excludes qualified symbols"
    (is (= (unqualified-symbols-in '(a longterm.partition-utils/foo b))
          '#{a b})))
  ...
  (is (= (unqualified-symbols-in '(a longterm.partition-utils/foo (b {:a :a :b c :d [d e]} 1 "hello")))
        '#{a b c d e}))))
