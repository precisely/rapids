...
(defn body-expr
  "Returns an expression that returns the value of the partition (or suspends)."
  [p]
  {:pre [(partition? p)]}
  (let [body (-> p :body)]
    (if (-> body count (> 1))
...
(defn ^{:arglists '([p f & args] [p suspending f & args])
  ...
  [p & args]
  {:pre [(partition? p)]}
  (let [[suspending f & args] (if (fn? (first args))
                                (list* (suspending? p) args)
                                args)]
    [suspending f args]
    (assert (boolean? suspending))
    (assert (fn? f))
    (letfn [(safe-update-body [body]
              (let [result (apply f body args)]
