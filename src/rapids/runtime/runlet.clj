...
(defn attach-child-run!
  [child-run index]
  (if-not (run? child-run)
    (throw (ex-info "Invalid argument to child-run"
             {:type :runtime-error
              :child-run child-run
              :index index})))
  (if-not (= (current-run :state) :running)
    (throw (ex-info "Attempt to attach to run not in running state"
             {:type :runtime-error
              :current-run (current-run)
              :child-run child-run
              :index index})))
  (if-not (not= *current-run-id* (:id child-run))
    (throw (ex-info "Attempt to attach run"
             {:type :runtime-error
              :run child-run})))
  (let [waits (:waits child-run)]
    (assert (-> waits (contains? *current-run-id*) not)
      (str "Duplicate attempt to wait on run. " *current-run-id* " is already waiting on " (:id child-run)))
    (.setKey child-run :waits (assoc waits *current-run-id* index))))

(defn detach-child-run!
  [child-run]
  (let [waits (:waits child-run)]
