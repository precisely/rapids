...
(defn interrupt-run!
  ...
  []
  (let [uuid (new-uuid)]
    (if (not= :running (current-run :state))
      (throw (ex-info "Attempt to interrupt run which is not in :running state"
               {:type :runtime-error
                :run  (current-run)})))
    (if-not (nil? (current-run :interrupt))
      (throw (ex-info "Attempt to interrupt run which is already interrupted"
               {:type :runtime-error
                :run  (current-run)})))
...
(defn attach-waiting-run!
  [waiting-run index]
  (if-not (run? waiting-run)
    (throw (ex-info "Expecting a run for waiting-run"
             {:type      :runtime-error
              :child-run waiting-run
              :index     index})))
  (if-not (= (current-run :state) :running)
    (throw (ex-info "Attempt to attach waiting run which is not in running state"
             {:type        :runtime-error
              :current-run (current-run)
              :child-run   waiting-run
              :index       index})))
  (if (= *current-run-id* (:id waiting-run))
    (throw (ex-info "Cannot attach current run to itself as a waiting run"
             {:type :runtime-error
              :run  waiting-run})))
  (let [waits (:waits waiting-run)]
    (if (-> waits (contains? *current-run-id*))
      (throw (ex-info "Duplicate attempt to wait on run."
               {:type        :runtime-error
                :current-run (current-run)
                :run         waiting-run})))
    (.setKey waiting-run :waits (assoc waits *current-run-id* index))))

(defn detach-waiting-run!
  [waiting-run]
  (let [waits (:waits waiting-run)]
    (.setKey waiting-run :waits (dissoc waits *current-run-id*))))
