...
(defn patch-upgrade-check
  ...
  (let [[level newnum oldnum] (v/version-change (:version new-flow) (:version old-flow))
    ...
    (assert (if (= level :patch) (> newnum oldnum)))
    (when requires-check?
...
(defn minor-upgrade-check [new-flow old-flow]
  (let [[level newnum oldnum] (v/version-change (:version new-flow) (:version old-flow))
        requires-check? (or (not level) (= level :minor) (= level :snapshot))]
    (assert (if (= level :minor) (> newnum oldnum)))
    (when requires-check?
      )))
...
(defn merge-flows
  ...
  (let [[f1-version f2-version] (map :version [flow1 flow2])
    ...
    (patch-upgrade-check new-flow old-flow)
    (minor-upgrade-check new-flow old-flow)
    (-> old-flow
