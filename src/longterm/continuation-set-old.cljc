...
;;;; together.
;
;(declare add continue create combine continuation-fn-def)
;
;;;; ContinuationSet
;
...
;  (ContinuationSet. {} {} {} #{}))
;
;(defn unresolved? [cset] (-> cset :unresolved count (> 0)))
;
;(defn unresolved-addresses
;  "A lazy sequence of addresses with unresolved bodies"
;  [cset]
;  (for [[k v] (:bodies cset) :when #(fn? v)] k))
;
;(defn resolved-body
;  [address]
;  (
;   (defn add
;     "Given a ContinuationSet add a continuation at the given address,
;     The final partition of a flow should be added with NO result-key.
;     This will cause the final form to return its value to the caller.
;     "
